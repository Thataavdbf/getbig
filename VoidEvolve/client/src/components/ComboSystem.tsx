...
export default function ComboSystem() {
  const { combo, updateComboTimer } = useCombo();
  ...
  const timerProgress = Math.max(0, Math.min(1, combo.timer / 3));
      ...
      {showCombo && (
        ...
        >
          <div className="flex items-center gap-2">
            <div className="text-2xl font-bold">{combo.count}x</div>
            <div className="text-purple-300 font-medium">Combo</div>
          </div>
          
          {/* Multiplier display */}
          <div className="text-sm text-yellow-300 mt-1">
            x{combo.multiplier.toFixed(1)} Multiplier
          </div>
